...
findPrimes([X | Xs], Ys) :-
    X > 0, isPrime(X,composite), findPrimes(Xs,Ys).


findPrimes([X | Xs], [X | Ys]) :-
    X > 0, isPrime(X,prime), findPrimes(Xs,Ys).
...
primes_range(M, N, Primes) :-
   upto(M,N,Range), findPrimes(Range,Primes).
...
tower(K, N, 1) :-
    K > N.
tower(K, N, R) :-
    K =< N, Kplus1 is K + 1, tower(Kplus1,N,Towerresult), Y is (K + N + 1) div K, R is Y * Towerresult.
...
findpath(node(Y, X, _), [Y | Ys]) :- findpath(X, Ys).

findpath(node(Y, _, R), [Y | Ys]) :- findpath(R, Ys).
